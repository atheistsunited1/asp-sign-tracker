...
describe('parseLegacyName / inferSignType', () => {
  ...
  it('infers sign types by keyword precedence and layer override', () => {
    ...
    expect(inferSignType({ name: 'NC - Jesus Saves billboard Lamar 0815' })).toBe('billboard')
    expect(inferSignType({ name: 'X', description: 'Yellow sign on a pole' })).toBe('sign')
    expect(inferSignType({ name: 'X', description: 'something', layerSignType: 'billboard' })).toBe('billboard')
...
describe('detectLayer', () => {
  it('maps My Maps layer names to kinds', () => {
    ...
    expect(detectLayer('Billboards')).toEqual({ kind: 'billboard', isMajorCampaign: false })
